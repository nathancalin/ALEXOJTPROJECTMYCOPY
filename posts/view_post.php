<?php 
if (isset($_GET['id'])) {
    $qry = $conn->query("SELECT p.*, u.username, u.avatar, u.commend, c.name as `category` 
                         FROM `post_list` p 
                ...
                <div class="container-fluid">
                    ...
                    <div style="line-height:1em" class="mb-3">
                        ...
                        <div class="py-1">
                            ...
                            <span class="<?= $commend == 1 ? 'text-primary' : '' ?>">
                                <?= $username ?>
                                <?php if ($commend == 1): ?>
                                    <span class="badge bg-primary text-white">Commended User</span>
                                <?php endif; ?>
                            </span>
                        </div>
                    </div>
                    ...
                    <div class="list-group comment-list mb-3 rounded-0">
                        <?php 
                        $comments = $conn->query("SELECT c.*, u.username, u.avatar, u.commend FROM `comment_list` c 
                                                 INNER JOIN `users` u ON c.user_id = u.id 
                                                 WHERE c.post_id ='{$id}' 
                                                 ORDER BY abs(unix_timestamp(c.date_created)) ASC");
                                ...
                                <div class="col-auto flex-shrink-1 flex-grow-1">
                                    <div style="line-height:1em">
                                        <div class="font-weight-bolder">
                                            <span class="<?= $row['commend'] == 1 ? 'text-primary' : '' ?>">
                                                <?= $row['username'] ?>
                                                <?php if ($row['commend'] == 1): ?>
                                                    <span class="badge bg-primary text-white">Commended User</span>
                                                <?php endif; ?>
                                            </span>
                                        </div>
...
<script>
    $(function() {
        ...
        $('#delete_post').click(function() {
            _conf("Are you sure you want to delete this post?", "delete_post", ['<?= isset($id) ? $id : '' ?>'])
        });
        $('#comment').summernote({
            height: "15em",
